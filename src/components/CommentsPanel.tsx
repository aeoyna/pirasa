...
export const CommentsPanel: React.FC<{ appId: string, appName: string, onClose?: () => void, isEmbedded?: boolean }> = ({ appId, appName, onClose, isEmbedded }) => {
    ...
    const contentSection = (
        <div className={`comments-container ${isEmbedded ? 'is-embedded' : ''}`} onClick={e => e.stopPropagation()}>
            <div className="comments-header">
                <h2>{isEmbedded ? 'コメント' : `${appName} のコメント`}</h2>
                {!isEmbedded && onClose && <button className="comments-close" onClick={onClose}>×</button>}
            </div>
    ...
    if (isEmbedded) return contentSection;
